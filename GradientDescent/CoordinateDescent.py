...
class CoordinateGradientDescent:
    def __init__(self, initial_x, current_x, prev_x,
        ...
        """ Initiate variables for function minimisation:
            ...
            Computational time is expected to increase in response to
    ...
    def calculate(self, equation, partial1, partial2):
        """ Minimise the selected function via Coordinate Descent Algorithm.
        ...
        while (self.iter < self.max_iters):
            ...
            self.current_x[0] = float(self.current_x[0]) \
                - self.lr * float(partial1(self.prev_x))
            self.step = float(equation(self.prev_x))
            ...
            while (float(equation(self.prev_x)) > float(
                equation(self.current_x))) and (
                    self.step > self.tolerance):
                self.prev_x = self.current_x.copy()
                self.current_x[0] = \
                    float(self.current_x[0]) - self.lr * float(
                        partial1(self.prev_x))
                self.step = float(equation(self.prev_x)) - float(
                    equation(self.current_x))
                self.iter += 1
                self.x_path[0][self.iter] = float(self.current_x[0])
                self.x_path[1][self.iter] = float(self.current_x[1])
            self.prev_x = self.current_x.copy()
            self.current_x[1] = float(self.current_x[1]) \
                - self.lr * float(partial1(self.prev_x))
            self.step = float(equation(self.prev_x)) - float(
                equation(self.current_x))
            self.iter += 1
            ...
            while (float(equation(self.prev_x)) > float(
                ...
                self.prev_x = self.current_x.copy()
                self.current_x[1] = \
                    float(self.current_x[1]) - self.lr * float(
                ...
                self.x_path[0][self.iter] = float(self.current_x[0])
                self.x_path[1][self.iter] = float(self.current_x[1])
                number_of_iters = str("Number of iterations before achieving\
                                      the minimum: %s" % self.iter)
                coordinate = str("Coordinates of the\
                                 minimum: %s" % self.current_x)
    ...
    def graph_full_coordinate(self, equation, partial1, partial2):
        ...
        ax3.plot(x_path_grid[0], x_path_grid[1], y3,
                 label='Coordinate Descent Path')
    ...
    def graph_partial_coordinate(self, equation, partial1, partial2):
        ...
        ax4.plot(x_path_grid[0], x_path_grid[1], y4, label='Gradient Descent')
