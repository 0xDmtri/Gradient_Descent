...
class NesterovAcceleratedGradient:
    def __init__(self, initial_x, current_x, prev_x, lr, tolerance,
                 step, max_iters, mom):
        ...
        self.mom = mom  # Momentum Coefficient
        self.d_mom = [0., 0.]
        self.iter = 0
    ...
    def calculate(self, equation, partial1, partial2):
        """ Minimise the selected function via Nesterov Algorithm.
        ...
        while (self.step > self.tolerance) and (self.iter < self.max_iters):
            ...
            self.current_x[0] = float(self.current_x[0]) + (
                self.mom * self.d_mom[0]) - self.lr * float(partial1(
                    self.prev_x + [x * self.mom for x in self.d_mom]))
            self.current_x[1] = float(self.current_x[1]) + (
                self.mom * self.d_mom[1]) - self.lr * float(partial2(
                    self.prev_x + [x * self.mom for x in self.d_mom]))
            self.d_mom[0] = float(self.current_x[0]) - float(self.prev_x[0])
            self.d_mom[1] = float(self.current_x[1]) - float(self.prev_x[1])
            self.prev_x = abs(float(equation(self.prev_x))
    ...
    @staticmethod
    def graph_interval_function(self, equation):
    ...
    def graph_full_nesterov(self, equation, partial1, partial2):
    ...
    def graph_partial_nesterov(self, equation, partial1, partial2):
        ...
        ax4.plot(x_path_grid[0], x_path_grid[1], y4,
                 label='Nesterov Accelerated Gradient Descent')
